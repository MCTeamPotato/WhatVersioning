...
public final class WhatVersioning {
    ...
    private static void loadConfig() {
        JsonConfig config = JsonConfig.create(MOD_ID, "1")
                .put("CustomVersioning", Lists.newArrayList(
                        "Minecraft 26H2",
                        "Minecraft 114.514",
                        "Minecraft 2/3/4/5",
                        "Minecraft blackwell",
                ...
                .initialize();
        VERSIONS = config.getList("CustomVersioning", String.class);
    ...
    public static final class ForgeEvents {
        ...
        public static void tick(ClientTickEvent.Post event) {
            if (CHANGE_TITLE.consumeClick()) {
                Minecraft minecraft = Minecraft.getInstance();
                if (minecraft.player != null && minecraft.player.isShiftKeyDown()) {
                    loadConfig();
                    System.out.println("WhatVersioning config has reloaded.");
                }
                CURRENT = VERSIONS.get(ThreadLocalRandom.current().nextInt(VERSIONS.size()));
